...
def create_model(path,lang):
    ...
    for a in itertools.combinations_with_replacement(set_alpha, 2):
        bigram = a[0]+a[1]
        if model.has_key(bigram):
            continue
        else:
            model[bigram] = math.log((float(lines.count(bigram))+1.0)/(float(lines.count(a[0]))+27.0))
...
def predict(file, model_en, model_es):
    ...
    for i in range(0, len(lines_en) - 2):
        bigram = lines_en[i] + lines_en[i + 1]
        if model_en.has_key(bigram):
            score_en+= model_en[bigram]
        else:
            score_en+=math.log(1.0/27.0)
    for i in range(0, len(lines_es) - 2):
        bigram = lines_es[i] + lines_es[i + 1]
